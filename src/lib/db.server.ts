...
export function insertInstance(row: InstanceRow): void {
	db.query(
       ...
       (id, name, source_path, workspace_path, host_port, container_id, remote_workspace_folder, status, error, created_at, bridge_token, remote_user, image_source, mode, terminal_split, config_migrated)
     VALUES ($id, $name, $source_path, $workspace_path, $host_port, $container_id, $remote_workspace_folder, $status, $error, $created_at, $bridge_token, $remote_user, $image_source, $mode, $terminal_split, $config_migrated)`
	).run({
		$id: row.id,
		$name: row.name,
