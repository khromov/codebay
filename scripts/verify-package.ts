 ...
 * built manifest carries disk paths. A build that works perfectly via `bun start` can
 * still be dead on arrival once installed somewhere else. Run it after `bun run build`;
 * the release workflow runs it right before `npm publish`.
 *
 * Checks: the SSR page renders, a static asset from ./public is served, no island SSR
 * errors are logged, publicDir shipped with the build, and no manifest disk path
 * escapes the package.
 */
...
try {
	...
	if (!wav?.ok) {
		fail(
			`/sounds/done.wav returned ${wav?.status ?? 'nothing'} — the packed tarball is not serving static assets`
		);
	}

	const output = log.join('');

	const islandErrors = output.match(/Island SSR error.*/g);
	if (islandErrors) {
		fail(
	...
	// Mochi 0.9+ never copies publicDir into the build — it reads ./public from disk on
	// every boot and warns when the build counted files the runtime can't find. That
	// means `public` must be in the `files` list; this catches it falling out.
	if (/publicDir "[^"]*" is missing or empty/.test(output)) {
		fail(
			`the installed package warned that publicDir is missing — is "public" still in package.json "files"?\n  ${output.match(/publicDir "[^"]*" is missing or empty.*/)?.[0]}`
		);
	}

	// 5. Every disk path in the shipped manifest must stay inside the package. Manifest v2
	//    stores these relative to .mochi/; anything absolute or escaping is a portability bug.
	const manifest: {
