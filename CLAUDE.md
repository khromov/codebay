...
1. `createInstance` validates the source (a **local folder** — copied — or a **Git repo URL** — cloned; `repo-url.ts` parses/normalizes the URL, `git.server.ts` does the clone and reads the live branch), validates the selected enabled agent, allocates a host port, writes a `creating` row with its immutable `agent`, and kicks off `boot()` in the background (the HTTP request returns immediately).
2. `boot()` (background): `copyWorkspace`/`cloneRepo` (copy skips `node_modules` but **keeps `.git`** so each instance retains its history/remote) → `writeOverrideConfig` (injects code-server, a staged `codebay-tmux` feature, and, for generated Codex configs, a staged `codebay-codex` feature; generates the selected agent's terminal launcher; seeds declared ports; and returns `image_source`) → `devcontainerUp` → the selected agent's post-up injection registry.
...
**Bridge / attention** (`src/lib/bridge.server.ts`): an in-memory, UI-only signal each container can raise — `done` (agent finished) or `waiting` (agent needs input) — to pulse its IDE tab. The selected agent's hooks `curl` `host.docker.internal:$PORT/api/bridge/attention` with the instance id, a per-instance `bridge_token`, and a state. Claude hooks merge into `settings.json`; Codex hooks are manager-owned groups in user-level `${CODEX_HOME:-$HOME/.codex}/hooks.json` and preserve unrelated hooks. The bridge route is **exempt from Basic Auth** and authenticates by token instead; `bridge_token` is stripped before rows reach the client.

**Container injections** (`src/container-injections/`): each post-up thing the manager installs is a self-contained `Injection` module exporting `apply()`, optional `auth.status()`, and optional `check()`. `resolveInjections(agent)` composes common git/tmux/GitHub/env injections with either the Claude set (`claude-code-credentials` or `claude-code-custom`, hooks, status line, aliases) or the Codex set (`codex-credentials`, `codex-attention-hooks`, `codex-alias`). That registry is the single source of truth for boot, health, and setup-UI auth chips. Codex auth copies only file-backed `${CODEX_HOME:-~/.codex}/auth.json` or runs `codex login --with-api-key`; it deliberately does not copy other Codex state. All container command execution goes through `execInContainer`, which carries secret input in a scrubbed exec environment value rather than argv.

_To add one:_ create `src/container-injections/<name>.ts` exporting an `Injection`, then add it to the appropriate branch of `resolveInjections(agent)`. Array order is apply order — keep `git-safe-directory` first since later git-touching steps depend on it.
...
**Persistence** (`src/lib/db.server.ts`): `bun:sqlite`, WAL mode. Tables: `instances` (one row per instance, including immutable `agent` and resolved `image_source`), `port_forwards`, and `options` (generic key/value settings, including enabled agents). State lives under `DATA_DIR`; the DB is `<DATA_DIR>/app.sqlite` and workspace copies live under `<DATA_DIR>/instances/<id>/`. Schema changes are numbered SQL files in `migrations/` (`0001_init.sql` … current `0005_instance_agent.sql`) and apply on open.
