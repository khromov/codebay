...
**Browser sign-in for the Claude token** (`src/lib/claude-setup-token.server.ts`): Settings can mint a manual Claude token by driving the host's `claude setup-token` under a PTY (`Bun.spawn`'s `terminal` option) instead of making the user open a terminal. It's a two-step flow — the CLI's OAuth redirect goes to `platform.claude.com`, not a loopback callback, so it ends at a "paste the code" prompt: the `claudeSetupStart` action spawns the CLI and returns once that prompt is on screen, `claudeSetupFinish` types the code in and stores the `sk-ant-oat…` token it prints, `claudeSetupCancel` kills the child. One session at a time, pinned to `globalThis` (`__codebaySetupToken`) with a 10-minute reaper. The CLI's screen is Ink-rendered and never shown to the user; only a few markers are matched out of it, and the parsers carry the two traps worth knowing (`*.isolated.test.ts` covers both): on-screen prose has its spaces drawn as cursor-move escapes, so markers are matched with whitespace removed, while the **token** is matched against the raw stream so escape sequences bound it rather than gluing the next word onto the secret.
