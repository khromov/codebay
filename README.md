...
Note that credentials are applied when a container is **created**. Start/Stop only cycle the existing container — after changing a token, use **Rebuild** on the instance card (or **Restart container** in the IDE header) to re-run the injections.
